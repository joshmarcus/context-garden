...
- The ER diagram declares `RUN ||--|| USAGE`, implying a mandatory one-to-one entity, contradicting the text and implementation where `usage` is an optional open mapping stored directly on `Run` (`ONTOLOGY.md`, system-boundary diagram).
