...
| `hosts/deadline.py`, `hosts/deadline_scheduler.py`, `hosts/locking.py` | independent absolute termination schedules and portable locks for durable host operations |
...
| `web/app.py`, `web/common.py`, `web/trust.py`, `web/artifacts.py` | `create_app` and the template environment; the `Hub` (its `lock` held only by `tick()`, a separate `action_lock` held only by an action so a button press never waits for a pass), the `Site` (base template context, board data) and shared helpers; the HTML sanitiser behind `render_md`, the origin check on POSTs, and the inert artifact boundary. Design files and run captures use a restrictive sandbox CSP and `nosniff`; only a small preview allowlist is shown inline, while unknown formats download without entering the operator origin. |
