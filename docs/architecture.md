...
| `routing.py`, `cli/routing.py` | read-only, redacted task-to-worker routing explanations and their diagnostic CLI commands; these views never claim capacity or confer authority |
| `restricted_data.py` | trusted local restricted-data authorization, dataset permission narrowing, egress checks, and bounded evidence export |
