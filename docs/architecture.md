...
| `plugins/__init__.py`, `plugins/manifest.py`, `plugins/registry.py`, `plugins/discovery.py`, `plugins/loading.py`, `plugins/lock.py`, `plugins/resources.py`, `plugins/capabilities.py`, `plugins/command.py`, `cli/plugins.py` | the plugin platform's public data model and activation boundary: immutable validated manifests, the read-only registry, entry-point enumeration without imports, explicit loading of configured installed plugins with namespaced validation, secret redaction and invocation provenance, compatibility locks that fence execution when installed plugins drift, versioned context resources copied into task workspaces, and bounded host-provider and runner-transport capability execution (`docs/plugin-contract.md`) |
...
| `pr_explanations.py` | source-versioned standalone HTML PR explanations, component relationships, and annotated highlighted code grounded in the selected revision |
...
| `members.py`, `cli/members.py` | private garden membership and installation-credential registry, shared role/project authorization decisions, and administrator-facing enrollment, rotation, revocation, and membership commands |
| `publication.py` | the default-empty project and field publication policy, safe public projection builder, and atomic projection export |
| `coordination.py`, `coordination_api.py` | legacy coordinator protocol retained for migration/history compatibility; enabled applications do not select it as multiplayer authority |
| `multiplayer_client.py` | the application-facing multiplayer boundary shared by scheduler and web workflows: selects the Git client, fails closed, and provides cached-view and conflict-aware projection primitives |
| `migration.py`, `cli/migration.py` | content-addressed standalone-to-multiplayer previews, resumable version-gated cutover, durable legacy-scheduler fencing, and consistent standalone recovery exports; the explicit operator commands that drive those transitions |
| `publication.py`, `web/public.py` | allowlisted, redacted public-garden snapshots and the isolated read-only application that serves them without access to private garden state |
| `planner.py`, `plants.py`, `notify.py`, `notification_adapters.py`, `host_identity.py`, `upgrade.py`, `worker_rollout.py`, `storage.py`, `config.py`, `configuration.py` | the planning prompt and import; the botanical drawings; legacy notification hooks and typed, durable notification delivery; host-alias and shared-text redaction boundary; the pinned controller install; durable safe rollout of published versions to existing workers; physical-volume discovery and free-space measurement; configuration layering and editable-setting policy metadata |
...
| `web/actions/` | the task-action registry (`web/actions/tasks.py`: one function per action, registered by name) and the other mutation routes (`web/actions/configuration.py` for stale-safe configuration saves, plus `control`, `phases`, `decisions`, `friction`, `defects`) |
| `tui/` | the Textual TUI |
...
`OPENROUTER_API_KEY`; the runner sends that one value to the adapter through a one-shot
descriptor after setup, while the ordinary worker and ancestor process environments remain
scrubbed. SSH launches first remove the key from the remote environment and substitute it
only into the transient input stream, never the saved remote script. Codex JSONL remains the raw
...
| group | keys | meaning |
|---|---|---|
| the PR | `pr_number`, `pr_draft`, `pr_base`, `pr_state`, `pr_updated_at`, `head_sha`, `review_decision`, `checks`, `failed_checks`, `ci_status`, `ci_failed_at`, `ci_reruns`, `last_polled` | what the last poll saw; `ci_status` is the normalized provider result bound to `head_sha` and refreshes even when `pr_updated_at` does not |
...
Each of those is built in or configured. An installed distribution can also provide capabilities
through `garden.plugins` (`docs/plugin-contract.md`). Discovery reads installed entry-point
metadata without importing plugin code; startup imports only explicitly configured plugins,
validates each namespaced configuration before constructing capabilities, and records immutable
configuration provenance when a capability is invoked.
