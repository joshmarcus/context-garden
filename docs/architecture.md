...
| `planner.py`, `plants.py`, `notify.py`, `notification_adapters.py`, `host_identity.py`, `upgrade.py`, `storage.py`, `config.py`, `configuration.py` | the planning prompt and import; the botanical drawings; legacy notification hooks and typed, durable notification delivery; host-alias and shared-text redaction boundary; the pinned install; physical-volume discovery and free-space measurement; configuration layering and editable-setting policy metadata |
