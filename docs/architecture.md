...
| `plugins/__init__.py`, `plugins/manifest.py`, `plugins/registry.py`, `plugins/discovery.py`, `plugins/loading.py`, `plugins/lock.py`, `cli/plugins.py` | the plugin platform's public data model and activation boundary: immutable validated manifests, the read-only registry, entry-point enumeration without imports, explicit loading of configured installed plugins with namespaced validation, secret redaction and invocation provenance, and the compatibility lock commands that fence execution when installed plugins drift (`docs/plugin-contract.md`) |
