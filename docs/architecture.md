...
| `plugins/__init__.py`, `plugins/manifest.py`, `plugins/registry.py`, `plugins/discovery.py`, `plugins/loading.py`, `plugins/lock.py`, `plugins/resources.py`, `cli/plugins.py` | the plugin platform's public data model and activation boundary: immutable validated manifests, the read-only registry, entry-point enumeration without imports, explicit loading of configured installed plugins with namespaced validation, secret redaction and invocation provenance, compatibility locks that fence execution when installed plugins drift, and versioned context resources copied into task workspaces (`docs/plugin-contract.md`) |
