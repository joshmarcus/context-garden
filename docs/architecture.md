...
| `multiplayer_client.py` | the local installation's authenticated coordinator client shared by scheduler and web workflows: fail-closed behavior, cached authoritative views and authenticated projection cache, protocol compatibility and reconnect diagnostics, revision-fenced commands, and conflict-aware local projection synchronization |
