...
| `coordination.py`, `coordination_api.py`, `multiplayer_client.py` | the authoritative multiplayer SQLite coordinator, its authenticated v1 HTTP protocol, and the local client: versioned task/phase authority, fencing leases, recoverable projection/provider ledgers, and atomic shared capacity/spend reservations |
