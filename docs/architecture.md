...
| `plugins/__init__.py`, `plugins/manifest.py`, `plugins/registry.py`, `plugins/discovery.py`, `plugins/loading.py` | the plugin platform's public data model and activation boundary: immutable validated manifests, the read-only registry, entry-point enumeration without imports, and explicit loading of configured installed plugins with namespaced validation, secret redaction and invocation provenance (`docs/plugin-contract.md`) |
...
| `coordination.py`, `coordination_api.py` | the authoritative multiplayer SQLite coordinator and its authenticated v1 HTTP protocol: versioned task/phase authority, fencing leases, recoverable projection/provider ledgers, and atomic shared capacity/spend reservations |
| `multiplayer_client.py` | the local installation's authenticated coordinator client, fail-closed behavior, cached authoritative view, authenticated projection cache, protocol compatibility checks, and reconnect diagnostics |
...
| `web/public.py` | the isolated anonymous read-only viewer, with a positive route policy and only an exported public projection directory as its data source |
