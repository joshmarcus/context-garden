...
| `multiplayer_client.py` | the fail-closed authenticated coordinator client and conflict-aware local projection cache used by each enrolled installation |
| `migration.py`, `cli/migration.py` | previewed, journaled standalone-to-multiplayer cutover and its administrator commands, including recovery status and quiescent standalone export |
| `publication.py`, `web/public.py` | the explicit-field public projection writer and the isolated read-only viewer application that serves only that projection, including replacement and revocation events |
