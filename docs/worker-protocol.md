...
tools, a private artifact boundary, and named evidence-export classes. The local runner checks
the policy before it writes the brief or launches the worker. Requested dataset access may only
...
bindings and enforcement evidence when the lease is acquired, renewed, and activated. GPU
evidence identifies each assigned device and reports that device's memory, vendor, and feature
set; missing or incompatible evidence for any assigned device fails closed. The
...
proved stopped or superseded by a stronger fence. Controller cleanup likewise stops or fences an
activated workload before releasing its host-service reservation, and retains recoverable lease
state when either step fails. Global, heavy-work, and host safety caps remain additional limits,
and repeated claims for one run/activity envelope are idempotent.
