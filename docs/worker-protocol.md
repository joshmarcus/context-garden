...
- **Worktree**: the local runner creates `.garden/worktrees/<id>` from `origin/<base>`
  ...
  creates or reuses its host-side worktree as described in its variant below. Remote runners
  beyond `ssh` are not implemented: the `runner: remote` claim, heartbeat and finish flow
  are deferred to CG-216.
