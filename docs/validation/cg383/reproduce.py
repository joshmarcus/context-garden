...
STAT_FIELDS = ("anon", "file", "shmem", "inactive_file", "active_file", "slab_reclaimable")
...
def main() -> int:
    ...
    parser.add_argument("--slot-command", nargs=argparse.REMAINDER)
    ...
    if min(args.cache_mib, args.reclaim_mib, args.slot_mib) <= 0:
        parser.error("allocation sizes must be positive")
    args.fixture_root = args.fixture_root.resolve(strict=True)
    filesystem = subprocess.check_output(
        ["findmnt", "-T", str(args.fixture_root), "-no", "FSTYPE,TARGET"], text=True
    ).strip()
    group = cgroup_path()
    cache_bytes = args.cache_mib * 1024 * 1024
    reclaim_bytes = args.reclaim_mib * 1024 * 1024
    slot_bytes = args.slot_mib * 1024 * 1024
    limits = {name: (group / name).read_text().strip() for name in ("memory.high", "memory.max", "memory.swap.max")}
    before = sample(group)
    with tempfile.TemporaryDirectory(prefix="cg383-file-cache-", dir=args.fixture_root) as directory:
        ...
        if args.slot_command:
            subprocess.run(args.slot_command, check=True)
        else:
    ...
    result = {
        ...
        "invocation": {"cache_mib": args.cache_mib, "reclaim_mib": args.reclaim_mib,
                       "slot_mib": args.slot_mib, "slot_command": args.slot_command},
        "fixture": {"root": str(args.fixture_root), "filesystem": filesystem, "cgroup": str(group), "limits": limits, "cache_bytes": cache_bytes,
