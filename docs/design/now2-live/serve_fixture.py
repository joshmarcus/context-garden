...
(root / 'garden.yaml').write_text(yaml.safe_dump({'name': os.environ.get('NOW_DEMO_NAME', 'Now 2 · demonstration'), 'max_parallel': 4, 'review_parallel': 2, 'resources': {'max_parallel': 2}, 'harness': 'codex', 'products': {'demo': {'repo': str(root), 'base_branch': 'main'}}}))
(root / 'demo').mkdir()
