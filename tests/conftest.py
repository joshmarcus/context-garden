...
class FakeGitHub:
    ...
    def create_pr(self, slug, head, base, title, body, draft=False, reviewers=None):
        self._n += 1
<<<<<<< HEAD
        pr = PRInfo(number=self._n, url=f"https://example.com/pull/{self._n}", state="OPEN", title=title, head=head, base=base, body=body, updated_at="t1", head_sha=f"head-{self._n}", head_repo=slug, is_draft=draft, author=self.me())
        self.prs[head] = pr
