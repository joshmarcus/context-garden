...
def test_interaction_replay_parser_accepts_option_like_nonce_as_a_value(nonce):
    parsed = interaction_replay.parse_args([
        "--out", "/tmp/replay", "--head", "head-a", f"--nonce={nonce}",
    ])
    assert parsed.nonce == nonce
    assert parsed.head == "head-a"
