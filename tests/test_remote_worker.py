...
def test_remote_api_auth_claim_heartbeat_finish_and_origin(garden, monkeypatch):
    ...
    assert "token" not in str(payload).lower() and "secret-token" not in str(payload)
    assert set(payload) >= {"repo", "branch", "base", "setup", "turn_cap", "env_allowlist"}

    beat = client.post(f"/api/runs/{run.run_id}/heartbeat", json={"transcript": "hello\n"}, headers=auth)
    assert beat.status_code == 200
    done = client.post(f"/api/runs/{run.run_id}/finish", json={"exit_code": 0, "final_text": "done", "result": {"status": "done"}, "usage": {"input_tokens": 2}, "cost_usd": 0.1, "pushed_head": "abc"}, headers=auth)
