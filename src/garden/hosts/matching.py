...
def match_worker(
    ...
    *,
    ...
    Ordering is stable and prefers the authored configuration order. Capacity/fairness is
    represented by the caller's busy set: once an instance becomes busy the next stable
    candidate wins. A pin is a hard constraint, never a way around authorization.
    """
    if held:
