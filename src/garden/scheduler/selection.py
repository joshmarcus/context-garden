...
def worker_candidates(tasks: dict[str, Task], state: Any, max_revisions: int,
                      stack: bool, edit_pending: Callable[[Task], bool],
                      default_after: Callable[[Task], str | None] | None = None) -> list[tuple[Task, str]]:
    ...
    return queue + [(t, "work") for t in ready(tasks, stack=stack, default_after=default_after)
                    if not edit_pending(t)]
