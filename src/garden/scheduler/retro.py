...
class RetroMixin:
    ...
    def _file_retro_features(self, phase: Phase, next_phase: str, rev: dict[str, Any], wt: Path,
        ...
        """Turn the reconciliation's `features` into draft task files inside the retro worktree,
        so they land with the same PR as the retro document. Skips whatever `resolve_features`
        flags as a duplicate; returns the filed list and the next free id number."""
