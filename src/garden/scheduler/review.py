...
class ReviewMixin:
    ...
    def dispatch_review(self, task: Task, work_run: Run | None = None, count_round: bool = True,
        ...
        if needs_interaction and not reusable_author_interaction and not replay_matches:
            ...
            replay_run.save()
            return replay_run
        replay_nonce = (str(replay.get("nonce") or "")
                        if needs_interaction and not reusable_author_interaction else "")
        ...
        controller_evidence = bool(capture_paths or (needs_interaction and not reusable_author_interaction))
