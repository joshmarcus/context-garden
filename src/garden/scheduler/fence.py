...
class FenceMixin:
    ...
    def _worker_write_evidence(transcript: str, repo: Path, rel: str,
        ...
        """Return structured transcript evidence that a worker named ``repo / rel``.

        A final answer is a worker claim, not audit evidence.  Tool calls and Codex JSONL
        command/file-change items came from the run itself, so they distinguish a worker write
        from the operator's CLI, web, or commit activity during the same window.
        """
        if not transcript:
            return []
        target = repo / rel
        ...
        for line in transcript.splitlines():
            ...
            for tool in FenceMixin._tool_uses(event):
                ...
                if name not in {"Edit", "Write", "Bash"} or not isinstance(inputs, dict):
                    continue
                if any(candidate in value for value in inputs.values() if isinstance(value, str)
    ...
    def _tool_uses(value: Any) -> list[dict[str, Any]]:
        """Extract Claude stream-json tool-use records from an event envelope."""
