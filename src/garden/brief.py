...
- Judge the task's goal and acceptance outcomes and report what you actually tested or inspected. Use `verified` when per-criterion rows help, especially for criterion-specific evidence or an unmet outcome; otherwise a clear non-empty attestation in `summary` or `notes` is sufficient. For a `done` result, the scheduler expands that attestation into bounded, criterion-addressable evidence rows before checks, PR rendering, and review. A genuinely unmet outcome is `{{"criterion": "...", "not_done": true, "reason": "<why>"}}`; never turn uncertainty into a pass. Do not write a generated verification checklist into `pr_body`.
