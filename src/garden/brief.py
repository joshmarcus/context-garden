...
- Address every acceptance criterion in your final response: state the outcome and the test, inspection, alternate evidence, or honest attestation that supports it, or say what remains unmet. `verified` rows are the clearest concise form, but exact quotations and JSON shape are not required; the scheduler also accepts clear criterion-addressable prose. If a criterion is false, infeasible, overly prescriptive, or no longer expresses the intended outcome, propose a justified replacement in `criteria_amended` and address that replacement. Do not silently drop a material outcome or turn uncertainty into a pass. Do not write the verification checklist into `pr_body`.
