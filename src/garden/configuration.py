...
CONFIG_FIELDS: dict[str, ConfigField] = {f.key: f for f in (
    ...
    _field("dependencies.default_after", "optional_string", None,
           "Dependency default: Stack starts eligible work on an open parent PR; Merge waits for completion. Unset keeps the legacy document-aware rule.",
           scopes=(ConfigScope.GLOBAL, ConfigScope.PROJECT), choices=("stack", "merge")),
    _field("browser.enabled", "boolean", False, "Allows browser-dependent tasks and visual validation where Playwright is supported.", scopes=(ConfigScope.GLOBAL, ConfigScope.PROJECT)),
    ...
    _field("retro.auto_start", "boolean", False, "Queues the native closing review when all phase gates pass."),
    _field("retro.personas", "list", [], "Roles used by automatically queued phase-closing reviews."),
    _field("retro.prerequisites", "mapping", {}, "Maps a phase key to phases which must already be closed before its closing review."),
    _field("retro.require_owner_approval", "boolean", False, "Requires closing_review_approved in phase frontmatter before auto-start."),
    _field("retro.allow_frozen", "boolean", False, "Allows automatic closing review while a phase freeze is in force."),
    ...
    _field("resources.disk_reserve_bytes", "integer", 20 * 1024**3, "Free physical-disk reserve retained before local work and staging.", apply=ApplyMode.RUNTIME, minimum=0, units="bytes"),
    _field("resources.operation_required_bytes", "integer", 0, "Conservative allocation reserved for each admitted local operation when no better estimate is available.", apply=ApplyMode.RUNTIME, minimum=0, units="bytes"),
    _field("resources.windows_backing_path", "string", "", "Optional Windows path on the physical volume backing this WSL distribution.", apply=ApplyMode.RUNTIME),
